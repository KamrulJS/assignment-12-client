  ...
  const { data: myPayment = [], refetch } = useQuery(
    ...
    async () => {
      const res = await fetch(
        `https://audiopair-server-main.vercel.app/myPayment/?email=${user?.email}`,
        {
          headers: {
    ...
    <div>
      ...
      <div className="my-10">
        <h1 className="font-bold text-xl text-center">
          Your previous payment history: {myPayment.length}
        </h1>
        <div className="text-center">
          {myPayment.map((pay, index) => (
            <p className="my-4" key={pay._id}>
              {index + 1}.TransactionId: {pay.transactionId} <br />
              Date:{pay.date} <br />
              Price:{pay.price}
            </p>
          ))}
